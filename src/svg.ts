...
export function renderSvg(scene: Scene, options: RenderOptions = {}): string {
  ...
  const edgeSvg = scene.edges.map((edge, index) => {
    ...
    const dash = edge.kind === "feedback" ? ` stroke-dasharray="4 6"`
      : edge.kind === "task" ? ` stroke-dasharray="10 5"`
      : edge.kind === "result" ? ` stroke-dasharray="14 4 3 4"`
      : "";
  ...
  const accessibleDescription = spec.meta.description ?? spec.meta.subtitle ?? `包含 ${scene.groups.length} 个分区、${scene.nodes.length} 个节点和 ${scene.edges.length} 条连线的系统图谱`;
  return `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="atlas-accessible-title atlas-accessible-description"><title id="atlas-accessible-title">${xml(spec.meta.title)}</title><desc id="atlas-accessible-description">${xml(accessibleDescription)}</desc>${defs}${background}${title}${groups}${edgeSvg}${edgeLabels}${hubSvg}${nodes}${nodePulses}${junctionSvg}${notes}${layeredDecorations}${corners}</svg>`;
}
